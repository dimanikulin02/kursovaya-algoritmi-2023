...
class GenerateLLVM(object):
    ...
    def _declare_print_function_string(self):
        self.printf = Function(self.module, FunctionType(IntType(32), [IntType(8).as_pointer()], var_arg=True),
                               name="printf")
...
class GenerateBlocksLLVM(object):
    ...
    def partition(self, block):
        for i in block.instructions:
            if i.__class__ == tuple:
                if i[0] == 'break':
                    after_loop = afterloops.pop()[0]
                    self.gen.branch(after_loop)
                    print([i])
                    return
                elif i[0] == 'continue':
                    after_loop = afterloops.pop()[1]
                    self.gen.branch(after_loop)
                    print([i])
                    return
                else:
    ...
    def visit_IfBlock(self, block):
        ...
        testvar = block.instructions[0].instructions[len(block.instructions[0].instructions) - 1][
            len(block.instructions[0].instructions[len(block.instructions[0].instructions) - 1]) - 1]
        # print(testvar)
...
def main():
   ...
   begin
    ...
    bloc = GenerateCode(parent_block=bloc, tree=tree, scope='global', IsMain=False, table={tree})
    print(prTr(bloc, 1))
